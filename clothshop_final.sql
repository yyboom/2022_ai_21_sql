...
-- cart selectall
SELECT ca.cartid, ca.userid, c.username As user_name, i.itemname AS item_name, i.itemprice AS item_price, ca.amount, (ca.amount * i.itemprice) AS total_price
FROM cart ca
...
SELECT cu.userid,cu.username, it.interestid AS interest_id, i.itemname AS interested_itemname 
    ...
    WHERE cu.userid="id01"
    NOT IN (cu.itemid);
    
-- delete item
ALTER TABLE item MODIFY itemid INT NOT NULL;
ALTER TABLE item DROP PRIMARY KEY;
DELETE FROM item WHERE itemid=1000;